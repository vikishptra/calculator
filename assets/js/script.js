  ...
  key.addEventListener("click", () => {
    ...
    // jika keynya reset, reset semua
        if (key.classList.contains("reset")) {
            var reset1= confirm('Apakah anda yakin ingin reset semua ? ')
            if(reset1 == true){
...
function reset1(){
    document.getElementById("button").style.backgroundColor = "#99FEFF"
 }
function reset2(){
    document.getElementById("button").style.backgroundColor = "#FF6D6D"
 }
function reset3(){
    document.getElementById("button").style.backgroundColor = "#2FDD92"
 }